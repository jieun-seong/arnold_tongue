...
REAL step(REAL x){
  REAL dummy;
  dummy = omega - lambda*SIN(r2pi*x)/r2pi;
...
REAL map(REAL x){
  REAL dummy;
  dummy = (REAL)(x+omega-lambda*SIN(r2pi*x)/r2pi);
...
REAL dfdx(REAL x) {return (REAL) (r1-lambda*COS(r2pi*x));}

REAL ddfdxx(REAL x) {return (REAL) (r2pi*lambda*SIN(r2pi*x));}

REAL dfdl(REAL x) {return (REAL) (-SIN(r2pi*x)/r2pi);}

REAL ddfdll(REAL x) {return r0;}

REAL ddfdldx(REAL x) {return (REAL) (-COS(r2pi*x));}
