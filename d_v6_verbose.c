  ...
  for(m=0; m<NMAX_der-NMIN_der+1; m++){
    ...
    for(lambda=lambdamin; lambda<lambdamax; lambda+=lambdastep){
      ...
      for(i=0; i<N-1; i++){
        ...
        #ifdef VERBOSE
          PRINTTOCHECK("i = ", i);
          PRINTTOCHECK("d1_lambda_array[i+1]", d1_lambda_array[i+1]);
          PRINTTOCHECK("d1_omega_array[i+1]", d1_omega_array[i+1]);
          PRINTTOCHECK("d2_lambda_array[i+1]", d2_lambda_array[i+1]);
          PRINTTOCHECK("d2_omega_array[i+1]", d2_omega_array[i+1]);
          PRINTTOCHECK("d2_omega_lambda_array[i+1]", d2_omega_lambda_array[i+1]);
        #endif
      }
